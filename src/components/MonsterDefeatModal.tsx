...
export const MonsterDefeatModal = () => {
  ...
  const monster = todayMonster
    ? MONSTERS.find((m) => m.id === todayMonster.monsterId)
    : null;

  // モーダルが開いたらAIコメントを取得
  useEffect(() => {
    if (!showMonsterDefeatModal || !monster || todayMonster?.defeatComment) return;
      ...
      try {
        ...
        const response = await fetch("/api/overall-coach", {
          ...
          body: JSON.stringify({
            ...
            yearlyGoal,
            monthlyGoal,
            systemHint: `プレイヤーが本日「${monster.name}」という心の魔物を撃退した！フィジカル・スキル・サッカーIQのどれかカテゴリでの修練量で魔物を追い払った。この快挙を称え、次の修練への熱いモチベーションを与える短いコメントをしてください。`,
          }),
        ...
        setMonsterDefeatComment("素晴らしい！諦めない君の心が魔物を打ち負かした。その調子で明日も突き進め！");
      } finally {
  ...
  if (!showMonsterDefeatModal || !monster || !todayMonster) return null;
